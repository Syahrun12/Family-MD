            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                        let chat = global.db.data.chats[m.chat]
                        let user = global.db.data.users[m.sender]
                        if (name != 'verify.js' && user && user verifikasi === false) return conn.sendButton(m.chat, Hallo {'@' + m.sender.split`@`[0], Kamu Belum Terdaftar DI Database Bot\nSilahkan Daftar Terebih Dahulu\nSilahkan Klik Button DIbawah Ini Untuk Terdaftar Ke Database Bot`, wm, m)
